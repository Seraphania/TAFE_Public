...
def translate(row, col):
    ...
    rows = translation["rows"][row]
    cols = translation["cols"][col]
    return (rows, cols)
...
def update_score(player, score, players):
    if score == "draw":
        players["x_player"]["draws"] += 1
        players["o_player"]["draws"] += 1
    elif player == players["x_player"]["name"]:
        players["x_player"]["wins"] += 1
        players["o_player"]["losses"] += 1
    else:
        players["o_player"]["wins"] += 1
        players["x_player"]["losses"] += 1
    return players

def game_loop():
    board = create_board()
    current_player = players["current_player"]
    symbol = players["x_player"]["symbol"]
    while True:
        ...
        row, col = translation["rows"][row], translation["cols"][col]
        ...
        if is_winner(symbol, board):
            ...
            update_score(current_player, "win", players)
            break
        if not has_free_spaces(board):
            print("It is a draw!")
            update_score(current_player, "draw", players)
            break
        current_player = switch(current_player, name_x, name_o)
        symbol = switch(symbol, 'X', 'O')

def play():
    # TODO: Set player names in the players dictionary - Done
    # TODO: Initialize current_player from the dictionary - Done
    ...
    players["x_player"]["name"], players["o_player"]["name"] = name_x, name_o

    players["current_player"] = name_x
    print(players)
